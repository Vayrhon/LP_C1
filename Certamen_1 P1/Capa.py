...
class CapaManager:
    ...
    def visualizar_capas(self):
    # Crear una ventana de tkinter para la visualización
        window = tk.Tk()
        window.title("Visualización de Capas")

        for num_capa, capa in enumerate(self.capas):
            frame = tk.Frame(window)
            frame.pack(side=tk.LEFT, padx=10, pady=10)
            
            # Crear un título para la capa
            titulo = tk.Label(frame, text=f"Capa {num_capa}")
            titulo.pack()

            for fila, fila_valores in enumerate(capa):
                fila_frame = tk.Frame(frame)
                fila_frame.pack()
                for columna, estado in enumerate(fila_valores):
                    ...
                    # Crear etiqueta para cada celda con color y letra correspondiente
                    label = tk.Label(fila_frame, text=estado, bg=color, width=4, height=2, font=('Helvetica', '10'))
